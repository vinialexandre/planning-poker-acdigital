...
export class MetodologiaController {
  constructor(private readonly metodologiaService: MetodologiaService) {}
  ...
  async buscar(): Promise<MetodologiaModel[]> {
    try {
      ...
      throw new HttpException(
        new Result('Não foi possivel realizar a chamada', false, null, error),
  ...
  @ApiOkResponse({ type: MetodologiaModel })
  @Post()
  async adicionarMetodologia(
    @Body() metodologia: MetodologiaModel,
  ): Promise<MetodologiaModel> {
    try {
      return this.metodologiaService.adicionarMetodologia(metodologia);
    } catch (error) {
      throw new HttpException(
        new Result('Não foi possivel realizar a alteração', false, null, error),
        HttpStatus.BAD_REQUEST,
      );
    }
  }
